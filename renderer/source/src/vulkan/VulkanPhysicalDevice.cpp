...
std::vector<VkPhysicalDevice> VulkanPhysicalDevice::getPhysicalDevices(VkInstance & instance)
{
    ...
    bool wasSucsess = VulkanInitializers::validate(vkEnumeratePhysicalDevices(
        instance,
        &device_count,
        nullptr
    ));
    ...
    if(!wasSucsess) return devices;

    wasSucsess = VulkanInitializers::validate(vkEnumeratePhysicalDevices(
        instance,
        &device_count,
        devices.data()
    ));
